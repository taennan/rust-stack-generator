...
use crate::{
    common::test_utils,
    admin_api_key::{
        database::{AdminApiKeyDB, AdminApiKeyDBTrait},
        entity::Model,
    },
};
use db_interface::{
    admin_api_key::{AdminApiKey, CreateAdminApiKeyInput, SearchAdminApiKeyInput, UpdateAdminApiKeyInput},
    result::DBError,
};
...
mod create {
    ...
    async fn it_returns_newly_created_model() {
        ...
        let actual = AdminApiKeyDB::new(connection)
            .create(CreateAdminApiKeyInput::default())
            .await
            .unwrap();
        let expected = AdminApiKey::from(model);
    ...
    async fn it_throws_not_found_on_unsuccessful_creation() {
        ...
        let result = AdminApiKeyDB::new(connection)
            .create(CreateAdminApiKeyInput::default())
            .await;
...
mod get_one {
    ...
    async fn it_returns_some_model_when_found() {
        ...
        let actual = AdminApiKeyDB::new(connection)
            .get_one(SearchAdminApiKeyInput::default())
            .await
            .unwrap();
        let expected = Some(AdminApiKey::from(model));
...
mod get_many {
    ...
    async fn it_returns_models_when_found() {
        ...
        let actual = AdminApiKeyDB::new(connection)
            .get_many(SearchAdminApiKeyInput::default())
            .await
            .unwrap();
        let expected = vec![AdminApiKey::from(model.clone()), AdminApiKey::from(model.clone())];
...
mod update {
    ...
    async fn it_returns_updated_model_on_successful_update() {
        ...
        let actual = AdminApiKeyDB::new(connection)
            .update(UpdateAdminApiKeyInput::default())
            .await
            .unwrap();
        let expected = AdminApiKey::from(model);
