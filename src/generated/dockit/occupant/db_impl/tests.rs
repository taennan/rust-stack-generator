...
use crate::{
    common::test_utils,
    occupant::{
        database::{OccupantDB, OccupantDBTrait},
        entity::Model,
    },
};
use db_interface::{
    occupant::{Occupant, CreateOccupantInput, SearchOccupantInput, UpdateOccupantInput},
    result::DBError,
};
...
mod create {
    ...
    async fn it_returns_newly_created_model() {
        ...
        let actual = OccupantDB::new(connection)
            .create(CreateOccupantInput::default())
            .await
            .unwrap();
        let expected = Occupant::from(model);
    ...
    async fn it_throws_not_found_on_unsuccessful_creation() {
        ...
        let result = OccupantDB::new(connection)
            .create(CreateOccupantInput::default())
            .await;
...
mod get_one {
    ...
    async fn it_returns_some_model_when_found() {
        ...
        let actual = OccupantDB::new(connection)
            .get_one(SearchOccupantInput::default())
            .await
            .unwrap();
        let expected = Some(Occupant::from(model));
...
mod get_many {
    ...
    async fn it_returns_models_when_found() {
        ...
        let actual = OccupantDB::new(connection)
            .get_many(SearchOccupantInput::default())
            .await
            .unwrap();
        let expected = vec![Occupant::from(model.clone()), Occupant::from(model.clone())];
...
mod update {
    ...
    async fn it_returns_updated_model_on_successful_update() {
        ...
        let actual = OccupantDB::new(connection)
            .update(UpdateOccupantInput::default())
            .await
            .unwrap();
        let expected = Occupant::from(model);
