...
use crate::{
    common::test_utils,
    org_user::{
        database::{OrgUserDB, OrgUserDBTrait},
        entity::Model,
    },
};
use db_interface::{
    org_user::{OrgUser, CreateOrgUserInput, SearchOrgUserInput, UpdateOrgUserInput},
    result::DBError,
};
...
mod create {
    ...
    async fn it_returns_newly_created_model() {
        ...
        let actual = OrgUserDB::new(connection)
            .create(CreateOrgUserInput::default())
            .await
            .unwrap();
        let expected = OrgUser::from(model);
    ...
    async fn it_throws_not_found_on_unsuccessful_creation() {
        ...
        let result = OrgUserDB::new(connection)
            .create(CreateOrgUserInput::default())
            .await;
...
mod get_one {
    ...
    async fn it_returns_some_model_when_found() {
        ...
        let actual = OrgUserDB::new(connection)
            .get_one(SearchOrgUserInput::default())
            .await
            .unwrap();
        let expected = Some(OrgUser::from(model));
...
mod get_many {
    ...
    async fn it_returns_models_when_found() {
        ...
        let actual = OrgUserDB::new(connection)
            .get_many(SearchOrgUserInput::default())
            .await
            .unwrap();
        let expected = vec![OrgUser::from(model.clone()), OrgUser::from(model.clone())];
...
mod update {
    ...
    async fn it_returns_updated_model_on_successful_update() {
        ...
        let actual = OrgUserDB::new(connection)
            .update(UpdateOrgUserInput::default())
            .await
            .unwrap();
        let expected = OrgUser::from(model);
