...
use crate::{
    common::test_utils,
    resource::{
        database::{ResourceDB, ResourceDBTrait},
        entity::Model,
    },
};
use db_interface::{
    resource::{Resource, CreateResourceInput, SearchResourceInput, UpdateResourceInput},
    result::DBError,
};
...
mod create {
    ...
    async fn it_returns_newly_created_model() {
        ...
        let actual = ResourceDB::new(connection)
            .create(CreateResourceInput::default())
            .await
            .unwrap();
        let expected = Resource::from(model);
    ...
    async fn it_throws_not_found_on_unsuccessful_creation() {
        ...
        let result = ResourceDB::new(connection)
            .create(CreateResourceInput::default())
            .await;
...
mod get_one {
    ...
    async fn it_returns_some_model_when_found() {
        ...
        let actual = ResourceDB::new(connection)
            .get_one(SearchResourceInput::default())
            .await
            .unwrap();
        let expected = Some(Resource::from(model));
...
mod get_many {
    ...
    async fn it_returns_models_when_found() {
        ...
        let actual = ResourceDB::new(connection)
            .get_many(SearchResourceInput::default())
            .await
            .unwrap();
        let expected = vec![Resource::from(model.clone()), Resource::from(model.clone())];
...
mod update {
    ...
    async fn it_returns_updated_model_on_successful_update() {
        ...
        let actual = ResourceDB::new(connection)
            .update(UpdateResourceInput::default())
            .await
            .unwrap();
        let expected = Resource::from(model);
