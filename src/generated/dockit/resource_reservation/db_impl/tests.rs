...
use crate::{
    ...
    resource_reservation::{
        database::{ResourceReservationDB, ResourceReservationDBTrait},
        entity::Model,
    },
};
use db_interface::{
    resource_reservation::{ResourceReservation, CreateResourceReservationInput, SearchResourceReservationInput, UpdateResourceReservationInput},
    result::DBError,
};
...
mod create {
    ...
    async fn it_returns_newly_created_model() {
        ...
        let actual = ResourceReservationDB::new(connection)
            .create(CreateResourceReservationInput::default())
            .await
            .unwrap();
        let expected = ResourceReservation::from(model);
    ...
    async fn it_throws_not_found_on_unsuccessful_creation() {
        ...
        let result = ResourceReservationDB::new(connection)
            .create(CreateResourceReservationInput::default())
            .await;
...
mod get_one {
    ...
    async fn it_returns_some_model_when_found() {
        ...
        let actual = ResourceReservationDB::new(connection)
            .get_one(SearchResourceReservationInput::default())
            .await
            .unwrap();
        let expected = Some(ResourceReservation::from(model));
...
mod get_many {
    ...
    async fn it_returns_models_when_found() {
        ...
        let actual = ResourceReservationDB::new(connection)
            .get_many(SearchResourceReservationInput::default())
            .await
            .unwrap();
        let expected = vec![ResourceReservation::from(model.clone()), ResourceReservation::from(model.clone())];
...
mod update {
    ...
    async fn it_returns_updated_model_on_successful_update() {
        ...
        let actual = ResourceReservationDB::new(connection)
            .update(UpdateResourceReservationInput::default())
            .await
            .unwrap();
        let expected = ResourceReservation::from(model);
