...
use crate::{
    ...
    resource_reservation_occupant::{
        database::{ResourceReservationOccupantDB, ResourceReservationOccupantDBTrait},
        entity::Model,
    },
};
use db_interface::{
    resource_reservation_occupant::{ResourceReservationOccupant, CreateResourceReservationOccupantInput, SearchResourceReservationOccupantInput, UpdateResourceReservationOccupantInput},
    result::DBError,
};
...
mod create {
    ...
    async fn it_returns_newly_created_model() {
        ...
        let actual = ResourceReservationOccupantDB::new(connection)
            .create(CreateResourceReservationOccupantInput::default())
            .await
            .unwrap();
        let expected = ResourceReservationOccupant::from(model);
    ...
    async fn it_throws_not_found_on_unsuccessful_creation() {
        ...
        let result = ResourceReservationOccupantDB::new(connection)
            .create(CreateResourceReservationOccupantInput::default())
            .await;
...
mod get_one {
    ...
    async fn it_returns_some_model_when_found() {
        ...
        let actual = ResourceReservationOccupantDB::new(connection)
            .get_one(SearchResourceReservationOccupantInput::default())
            .await
            .unwrap();
        let expected = Some(ResourceReservationOccupant::from(model));
...
mod get_many {
    ...
    async fn it_returns_models_when_found() {
        ...
        let actual = ResourceReservationOccupantDB::new(connection)
            .get_many(SearchResourceReservationOccupantInput::default())
            .await
            .unwrap();
        let expected = vec![ResourceReservationOccupant::from(model.clone()), ResourceReservationOccupant::from(model.clone())];
...
mod update {
    ...
    async fn it_returns_updated_model_on_successful_update() {
        ...
        let actual = ResourceReservationOccupantDB::new(connection)
            .update(UpdateResourceReservationOccupantInput::default())
            .await
            .unwrap();
        let expected = ResourceReservationOccupant::from(model);
